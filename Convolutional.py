...
class Convolutional2D:
    def __init__(self, num_kernels: int, kernel_depth: int, kernel_width: int, kernel_height: int, output_depth: int, output_width: int, output_height: int, Optim=DefaultOptim):
        self.k = Tensor(np.random.randn(num_kernels, kernel_depth, kernel_width, kernel_height), trainable=True, Optim=Optim)
        self.b = Tensor(np.random.randn(1, output_depth, output_width, output_height), trainable=True, Optim=Optim)
